...
test.describe("Product List API", () => {
  test("Should retrieve all products", async ({ productService }) => {
    ...
    if (productsLength > 0) {
      ...
      BaseValidator.validateFieldValueNotEmpty(
        response,
      ...
      BaseValidator.validateArrayHasNoDuplicateObjects(response, "products");
    }
  });
  ...
  test("Should retrieve not supported method error when using unsupported HTTP method: POST", async ({
    ...
    BaseValidator.validateStatusCode(response, successCode);
    BaseValidator.validateFieldValue(response, "responseCode", 405);
    BaseValidator.validateErrorResponse(
      response,
...
test.describe("Brand List API", () => {
  test("Should retrieve all brands of products", async ({ productService }) => {
    ...
    BaseValidator.validateStatusCode(response, successCode);
    BaseValidator.validateSchema(response, brandListSchema);
    BaseValidator.validateFieldValue(response, "responseCode", successCode);
    ...
    if (brandsLength > 0) {
      ...
      BaseValidator.validateArrayHasNoDuplicateObjects(response, "brands");
    }
  });
  ...
  test("Should retrieve not supported method error when using unsupported HTTP method: POST", async ({ apiClient }) => {
    ...
    BaseValidator.validateStatusCode(response, successCode);
    BaseValidator.validateFieldValue(response, "responseCode", 405);
    BaseValidator.validateErrorResponse(response, "This request method is not supported.");
  });
...
test.describe("Search Product API", () => {
  ...
  const records: SearchProductRecord[] = getDataFromJsonFile<SearchProductRecord[]>("api/searchProduct.json");
  for (const record of records) {
    test(`Should retrieve searched products by name "${record.keyword}"`, async ({ productService }) => {
      ...
      BaseValidator.validateFieldValue(response, "responseCode", successCode);
      if (!record.hasResult) {
        BaseValidator.validateFieldValueIsEmptyArray(response, "products");
      } else {
        const productsLength = response.body.products.length;
        console.log(`==> Number of products retrieved for search keyword "${record.keyword}": ${productsLength}`);
        BaseValidator.validateFieldValueIsNonEmptyArray(response, "products");
        BaseValidator.validateArrayHasNoDuplicateObjects(response, "products");
        for(let i = 0; i < productsLength; i++) {
          const expected = record.keyword;
          BaseValidator.validateFieldValueContainsText(response, `products[${i}].name`, expected, { soft: true });
        }
      }
    });
  }
  ...
  test("Should retrieve all products when search product name is empty", async ({ apiClient }) => {
    ...
    BaseValidator.validateStatusCode(response, successCode);
    BaseValidator.validateFieldValue(response, "responseCode", 200);
    BaseValidator.validateFieldValueIsNonEmptyArray(response, "products");
    BaseValidator.validateArrayHasNoDuplicateObjects(response, "products");
  });
  ...
  test("Should retrieve error when lacking search product name", async ({ apiClient }) => {
    ...
    const response = await apiClient.post(endpoint, payloadData);
    BaseValidator.validateStatusCode(response, successCode);
    BaseValidator.validateFieldValue(response, "responseCode", 400);
    BaseValidator.validateErrorResponse(response, "Bad request, search_product parameter is missing in POST request.");
  });
